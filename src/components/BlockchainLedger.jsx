    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Immutable SHA-256 Merkle Evidence Trail & Smart Contract Audit Log (<code className="text-emerald-300">BlueCarbonMRV.sol</code>)
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 text-slate-950 font-extrabold text-xs hover:opacity-95 transition-all flex items-center space-x-2 shadow-lg shadow-emerald-500/20"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-3">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
            <h3 className="text-xs font-extrabold uppercase text-slate-300 flex items-center gap-1.5">
              <Hash className="w-4 h-4 text-emerald-400" /> SHA-256 Evidence Payload Hash
            </h3>
          ...
          <div className="text-[11px] text-slate-400 space-y-1">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-emerald-400 font-bold">MATCHED (0 Discrepancies)</span>
            </div>
          </div>
        </div>
        ...
        <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-3">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
            <h3 className="text-xs font-extrabold uppercase text-slate-300 flex items-center gap-1.5">
              <Layers className="w-4 h-4 text-cyan-400" /> Cryptographic Merkle Root Proof
            </h3>
          ...
          <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 font-mono text-[11px] text-cyan-300 break-all">
            0x{merkleRootHash}
          </div>
          ...
          <div className="text-[11px] text-slate-400 space-y-1">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-cyan-400 font-mono">BlueCarbonMRV.sol</span>
            </div>
          </div>
        </div>
        ...
        <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-3 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-800 pb-2">
              <h3 className="text-xs font-extrabold uppercase text-slate-300 flex items-center gap-1.5">
                <Lock className="w-4 h-4 text-purple-400" /> Blockchain Network Stats
              </h3>
            ...
            <div className="space-y-2 mt-3 text-xs">
              ...
              <div className="flex justify-between text-slate-300">
                ...
                <span className="font-mono text-emerald-400 font-bold">2.1 Seconds</span>
              </div>
              <div className="flex justify-between text-slate-300">
                <span>Carbon Credit Standard:</span>
                <span className="font-mono text-purple-300 font-bold">ERC-1155 Environmental</span>
              </div>
            </div>
          </div>
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
          <h3 className="text-sm font-extrabold text-white flex items-center gap-2">
            <ExternalLink className="w-4 h-4 text-emerald-400" /> Smart Contract Transaction Log
          </h3>
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono">
              {filteredBlocks.map((tx, idx) => (
                <tr key={idx} className="hover:bg-slate-800/40 transition-colors">
                  <td className="p-3 text-emerald-400 font-bold">#{tx.block}</td>
                  <td className="p-3 text-slate-400">{tx.timestamp}</td>
                  <td className="p-3 text-white font-sans font-semibold">{tx.event}</td>
                  <td className="p-3 text-cyan-300">{tx.txHash.substring(0, 16)}...</td>
                  <td className="p-3 text-slate-400">{tx.signer}</td>
                  <td className="p-3 text-purple-300">{tx.gasUsed}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showCertificateModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="glass-panel p-8 rounded-3xl border border-emerald-500/40 max-w-2xl w-full shadow-2xl space-y-6 relative animate-scale-up">
            ...
            <div className="grid grid-cols-2 gap-4 text-xs bg-slate-950 p-5 rounded-2xl border border-slate-800">
              <div>
                <span className="text-slate-500 uppercase text-[10px] font-bold block">Project Name</span>
                <strong className="text-white text-sm">{selectedSite.name}</strong>
              </div>
              <div>
                <span className="text-slate-500 uppercase text-[10px] font-bold block">State / Region</span>
                <strong className="text-emerald-400 text-sm">{selectedSite.state}</strong>
              </div>
              <div>
                <span className="text-slate-500 uppercase text-[10px] font-bold block">Total Verified Carbon</span>
                <strong className="text-emerald-400 text-sm font-mono">
                  {selectedSite.carbonStock.totalCO2e.toLocaleString()} tCO₂e
                </strong>
              </div>
              <div>
                <span className="text-slate-500 uppercase text-[10px] font-bold block">Annual Sequestration</span>
                <strong className="text-cyan-300 text-sm font-mono">
                  {selectedSite.carbonStock.annualSequestrationHa} t/ha/year
                </strong>
              </div>
              <div className="col-span-2 pt-2 border-t border-slate-800">
                <span className="text-slate-500 uppercase text-[10px] font-bold block">Immutable Blockchain Hash</span>
                <code className="text-[11px] text-cyan-300 font-mono break-all">
                  0x{evidenceHash}
                </code>
              </div>
            </div>
