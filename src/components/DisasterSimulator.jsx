...
export default function DisasterSimulator({ 
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Header Banner */}
      <div className="glass-panel p-6 rounded-2xl border border-red-500/30 bg-gradient-to-r from-slate-950 via-slate-900 to-red-950/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-2xl">
        ...
        {activeDisaster ? (
          <button
            ...
            className="px-4 py-2 rounded-xl bg-emerald-500 text-slate-950 font-bold text-xs hover:bg-emerald-400 transition-all flex items-center space-x-2 shadow-lg shadow-emerald-500/20"
          >
            <RotateCcw className="w-4 h-4" />
            <span>Reset Ecosystem Baseline</span>
          </button>
        ) : (
          <div className="text-xs text-slate-400 bg-slate-900/80 px-4 py-2 rounded-xl border border-slate-800 flex items-center space-x-2">
          ...
          return (
            <div
              ...
              className={`glass-panel p-5 rounded-2xl border cursor-pointer transition-all duration-300 relative overflow-hidden group ${
                isActive
                  ? `border-red-500 bg-red-950/40 shadow-2xl ring-2 ring-red-500/50`
                  : `border-slate-800 hover:border-slate-700 bg-slate-950/60 hover:bg-slate-900/80`
              }`}
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center space-x-3">
                  <div className={`w-10 h-10 rounded-xl bg-gradient-to-tr ${scenario.color} flex items-center justify-center shadow-md`}>
                ...
                {isActive && (
                  <span className="px-2.5 py-1 text-[10px] font-black uppercase tracking-wider bg-red-500 text-white rounded-full animate-pulse">
                    ACTIVE NOW
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 mt-4 pt-3 border-t border-slate-800/80 text-[11px]">
                ...
                <div className="bg-slate-900/80 p-2 rounded-xl text-center">
                  <span className="text-slate-400 block text-[10px]">NDVI Drop</span>
                  <strong className="text-orange-400 text-xs font-mono">-{scenario.impactNdviDrop}</strong>
                </div>
                <div className="bg-slate-900/80 p-2 rounded-xl text-center">
                  <span className="text-slate-400 block text-[10px]">Est. CO₂ Loss</span>
                  <strong className="text-purple-300 text-xs font-mono">{scenario.carbonLossEstimate}</strong>
                </div>
              </div>
      ...
      {activeDisaster && (
        <div className="glass-panel p-5 rounded-2xl border border-red-500/50 bg-red-950/20 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
            <div className="p-3 bg-slate-950/80 rounded-xl border border-red-500/30">
              <span className="text-slate-400 block text-[10px] uppercase font-bold">1. Digital Twin Reaction</span>
            ...
            <div className="p-3 bg-slate-950/80 rounded-xl border border-red-500/30">
              <span className="text-slate-400 block text-[10px] uppercase font-bold">2. AI Reversal Risk Engine</span>
              <p className="text-slate-200 mt-1">
                Risk score jumped from <strong className="text-emerald-400">{selectedSite.reversalRisk.score}%</strong> to <strong className="text-red-400 font-extrabold">{activeDisaster.impactRiskScore}% (CRITICAL)</strong>.
              </p>
            </div>
            <div className="p-3 bg-slate-950/80 rounded-xl border border-red-500/30">
              <span className="text-slate-400 block text-[10px] uppercase font-bold">3. Blockchain Action</span>
              <p className="text-slate-200 mt-1">
                Smart contract emitted <code className="text-cyan-300 font-mono">{activeDisaster.blockchainEvent}</code>. Carbon credit issuance frozen.
              </p>
            </div>
          </div>
        </div>
